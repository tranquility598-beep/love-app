...
function startDirectCall(partnerName, partnerAvatar, isVideo = false) {
    // 1. Сброс состояний
    isCallMuted = false;
    ...
    // 4. Показ панели — ТОЛЬКО голосовой профиль по умолчанию; видео-сетка только если isVideo
    ...
    if (callDurationText) callDurationText.textContent = "Вызов...";
...
if (actionCall) {
    actionCall.addEventListener("click", () => {
        ...
        startDirectCall(name, avatar, false);
    });
}
if (actionVideo) {
    actionVideo.addEventListener("click", () => {
        const name = document.getElementById("header-name")?.textContent.trim() || "Мария";
        const avatar = document.getElementById("header-avatar")?.textContent.trim() || "М";
        startDirectCall(name, avatar, true);
    });
}
...
function showServerVoice(channelName) {
    ...
    if (voicePanel) {
        ...
        // Симуляция входа пользователей
        // Инициализируем 3-мя пользователями (Вы, Мария, Иван)
        voiceMembers = [
            { name: "Founder", avatar: "F", speaking: false, hasCam: false, isOwn: true, micActive: true, soundActive: true },
            { name: "Мария", avatar: "М", speaking: true, hasCam: false, isOwn: false, micActive: true, soundActive: true },
            { name: "Иван", avatar: "И", speaking: false, hasCam: false, isOwn: false, micActive: false, soundActive: true }
        ...
        // Через 3 секунды заходит Анна (4-й пользователь)
        window.voiceSimTimer1 = setTimeout(() => {
            if (!voicePanel.classList.contains("hidden")) {
                voiceMembers.push({ name: "Анна", avatar: "А", speaking: false, hasCam: false, isOwn: false, micActive: true, soundActive: true });
                renderVoiceChannel();
                showToast("Голосовая связь", "Анна подключилась к каналу.");
            }
        }, 3000);

        // Через 6 секунд заходит Дмитрий (5-й пользователь)
        window.voiceSimTimer2 = setTimeout(() => {
            if (!voicePanel.classList.contains("hidden")) {
                voiceMembers.push({ name: "Дмитрий", avatar: "Д", speaking: true, hasCam: false, isOwn: false, micActive: true, soundActive: true });
                renderVoiceChannel();
                showToast("Голосовая связь", "Дмитрий подключился к каналу.");
            }
        }, 6000);
...
function loadFriends(type) {
    ...
    list.forEach(friend => {
        ...
        card.querySelector(".call-action").addEventListener("click", () => {
            startDirectCall(friend.name, friend.avatar, false);
        });
...
let mockHubUpdates = [
    {
        ...
        date: "09.06.2026",
        tag: "Major Release",
        title: "Обновление Love App v5.2.0",
        desc: "Мы полностью переработали визуальный стиль приложения. Улучшена плавность анимаций модальных окон, добавлены кастомные SVG-иконки и умный виджет в системном трее.",
        changes: [
            "Полный редизайн интерфейса с использованием концепции Bento Grid.",
            "Плавная анимация появления модальных окон и переходов.",
            "Новые SVG-иконки для закрепления чатов и навигационной панели.",
            "Интеграция подсказок и туториалов для новых функций."
        ]
    },
    {
        id: 2,
        ...
        date: "01.06.2026",
        tag: "Minor Update",
        title: "Улучшения производительности v5.1.5",
        desc: "В этом обновлении мы сфокусировались на стабильности голосовых комнат и ускорении загрузки медиа-файлов.",
        changes: [
            "Оптимизация кодека Opus для более чистого звука при слабом сигнале сети.",
            "Исправлена утечка памяти при длительном прослушивании трансляций.",
            "Кэширование аватаров пользователей для ускорения рендеринга списков."
        ]
    },
    {
        id: 3,
        ...
        date: "20.05.2026",
        tag: "Minor Update",
        title: "Голосовые комнаты и стриминг v5.1.0",
        desc: "Добавлена поддержка комнат для совместного прослушивания и улучшен режим кинотеатра.",
        changes: [
            "Запуск новой вкладки 'Голосовые сетки' для общения.",
            "Режим кинотеатра с поддержкой демонстрации экрана.",
            "Регулировка громкости для отдельных участников комнаты."
        ]
    },
    {
        id: 4,
        ...
        date: "01.05.2026",
        tag: "Major Release",
        title: "Глобальное обновление Love App v5.0.0",
        desc: "Переход на новую архитектуру ядра, обеспечивающую мгновенную доставку сообщений и повышенную безопасность.",
        changes: [
            "Новый протокол шифрования сообщений.",
            "Внедрение системы локального хранения истории чатов.",
            "Создание раздела Love Hub для связи с сообществом."
        ]
    }
];
...
function initContextMenu() {
    ...
    document.addEventListener("contextmenu", (e) => {
        ...
        if (pinAction || unpinAction) {
            ...
            menuItem.innerHTML = `${actionIcon} <span>${label}</span>`;
