...
//
// Зашитый номер здесь один на весь интерфейс — на него падаем, только если
// Electron не ответил (например, страница открыта в браузере). Строку сверяет
// ops/check-release-version.cjs с package.json, чтобы она не отстала от релиза.
const APP_VERSION_FALLBACK = '2.1.1';
const APP_VERSION = (window.electronAPI && typeof window.electronAPI.getVersion === 'function')
    ? (window.electronAPI.getVersion() || APP_VERSION_FALLBACK)
    : APP_VERSION_FALLBACK;
window.LOVE_APP_VERSION = APP_VERSION;
...
// У прошлых выпусков номер зашит: подставлять туда версию сборки нельзя, иначе
// старая запись подпишется свежим номером.
let mockHubUpdates = [
    {
        id: 10,
        ...
        desc: "Приложение называло себя 2.0.0 — и в настройках, и в Love Hub, и в этом самом списке. Обновлялось оно при этом честно, врала только надпись.",
        changes: [
            "«Текущая версия» в настройках, «О программе» и заголовок Love Hub берут номер у самой сборки, а не показывают заглушку",
            "Верхняя запись в истории обновлений больше не подписывается номером свежего выпуска",
            "Сборка релиза теперь не соберётся, если номер версии забыли обновить хоть в одном месте"
        ]
    },
    {
        id: 1,
