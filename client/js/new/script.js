...
const APP_VERSION = (window.electronAPI && typeof window.electronAPI.getVersion === 'function')
    ? (window.electronAPI.getVersion() || '2.0.2')
    : '2.0.2';
...
let mockHubUpdates = [
    {
        ...
        desc: "Стабильность: войс и демонстрация экрана через свой TURN, регистрация, личные чаты.",
        changes: [
            "Свой TURN-сервер — войс в ЛС и демонстрация экрана теперь стабильно соединяются",
            "Починена регистрация (ввод ника больше не кидает на вход)",
            "Письма (код подтверждения, сброс пароля) через Resend + новый стильный дизайн",
            "Личный чат открывается прямо из «Друзей» и из профиля",
            "Восстановление пароля по коду на почту",
            "Восстановлена Android-сборка (CI на Node 22 + Java 21)"
        ]
    }
];
